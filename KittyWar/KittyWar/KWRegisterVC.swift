...
class KWRegisterVC: KWAlertVC {
    ...
    @IBAction func register(_ sender: UIButton) {
        ...
        KWNetwork.shared.webServerRegister(username: username, email: email, password: password)
    }

    func handleRegisterResult(notification: Notification) {
        if let result = notification.userInfo?[WebServerResponseKey.result] as? RegisterResult {
            switch result {
